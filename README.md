...
[!WARNING] pycryptodome can conflict with the deprecated Crypto package
...
[!WARNING] The server ip needs to be a local ip on the same network as the client unless you want to deal with port forwarding the server, which is untested
...
[!WARNING] If no password is provided, the default password of "testPass" will be used, whicch is obviously not secure
...
[!NOTE] The load times in firefox with the proxy tend to be quite slow